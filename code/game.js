...
var actorChars = {
  ...
  "=": slime, "|": slime, "v": slime,
  "p": mfloater, "u":mfloater, "i": mfloater,
  //portal slime
  "+": pslime, "-":pslime, "*": pslime,
  "s":spikes
};
...
// slime is initialized based on the character, but otherwise has a
// size and position
...
mfloater.prototype.type = "mfloater";

// portalslime
function pslime(pos, ch) {
  this.pos = pos;
  this.size = new Vector(1, 1);
  if (ch == "+") {
    // Horizontal lava
...
var wobbleSpeed = 30, wobbleDist = 0.10;

pslime.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
...
mfloater.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
...
var wobbleSpeed = 10, wobbleDist = 0.08;
...
var wobbleSpeed = 30, wobbleDist = 0.07;
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  if (obstacle || (actor&& actor.type == "mfloater"))
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  // if the player touches f and the player hasn't won
  // Player loses
